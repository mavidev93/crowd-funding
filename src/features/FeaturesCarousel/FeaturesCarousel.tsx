...
const FeaturesCarousel = (props: Props) => {
  const CustomRightArrow = ({ onClick, ...rest }: ArrowProps) => {
    return (
      <IconButton
        className="  bg-gray-200 	hover:bg-gray-300 absolute right-3 "
        onClick={() => onClick!()}
      >
        {" "}
        <KeyboardArrowRightRoundedIcon className="text-black" />{" "}
      </IconButton>
    );
  };
  ...
  const CustomLeftArrow = ({ onClick, ...rest }: ArrowProps) => {
    return (
      <IconButton
        className="  bg-gray-200 	hover:bg-gray-300 absolute left-3 "
        onClick={() => onClick!()}
      >
        {" "}
        <KeyboardArrowLeftRoundedIcon className="text-black" />{" "}
      </IconButton>
    );
  };
...
const dummySingleItems = [
  ...
  {
    title: "canvas",
    text: "t. Aspernatur ipsa dolorum tenetur enim molestias corrupti laudantium eum optio, illo quia maiores, dolorem, abexpedita voluptatum voluptatem? Dignissimos ea at magnam eligendi enim. Accusamus, dolor ab",
    date: format(new Date(), "yyyy MM dd"),
    id: nanoid(),
    amount: "5 ETH",
  },
