...
const CreatedCampaigns = (props: Props) => {
  ...
  const campaigns = useGetCampaigns(contractData?.contract, contractData?.abi, ids);


  //Contract Functions
  const { runContractFunction: withdrawFunds } = useWeb3Contract({
    abi: contractData?.abi,
    contractAddress: contractData?.crowdFundAddress ? crowdFundAddress : "",
  ...
  const { runContractFunction: closeCampaign } = useWeb3Contract({
    ...
    contractAddress: crowdFundAddress ? crowdFundAddress : "",
