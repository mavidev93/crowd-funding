...
const createContractFunction = (abi:object | undefined,contractAddress:string,functionName:string)=>{
...
export const getCampaigns = async ( contract:Contract,funder:string|null,owner:string|null,showDuplicated:boolean) => {
  ...
  try {
    ...
    if (allIds) {
      ...
      const getHashById = createContractFunction(abi,contractAddress,"getHashById");
      const getCampaignByHash = createContractFunction(abi,contractAddress,"getHashById");
