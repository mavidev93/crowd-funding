...
export const getCampaigns = async ( contract:Contract,funder:string|null,owner:string|null,showDoplicated:boolean,account: string) => {
  ...
  try {
    ...
    if (allIds) {
      ...
      const ids = showDoplicated ? allIds :[...new Set(allIds)] ;
