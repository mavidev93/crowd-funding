...
export default function FundModal({ campaignHash, isCampaignOpen }: Props) {
  ...
  const { runContractFunction: fundCampaign, error: fundError } =
    useWeb3Contract({
      abi: abi,
  ...
  const handleFund = async (e: any, fundVal: string) => {
    console.log(typeof fundVal);
    try {
await fundCampaign({
       async onSuccess(tx:any) {
 const txReceipt = await getRecept(tx)
      ...
      >
        <Box sx={style}>
          ...
          >
            ...
            />
            <Typography  sx={{color:"red",fontSize:"10px"}} component="p">Minimum amount is 0.001 ETH</Typography>
            <CommonButton
              text="Fund &#8594;"
              disabled={!isCampaignOpen}
              variant="contained"
