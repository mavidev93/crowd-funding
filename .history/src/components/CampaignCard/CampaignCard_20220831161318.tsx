...
const CampaignCard = ({ campaignIpfs }: Props) => {
  ...
  const handleAddToBookmark=()=>{
    ...
    if(bookmarks){
      localStorage.setItem("bookmarks",JSON.stringify([...bookmarks,{campaignTitle:"",campaignHash:""}]))
      console.log(true)
    }
    else{
      localStorage.setItem("bookmarks",JSON.stringify([{campaignTitle:"",campaignHash:""}]))
      ...
      <div className="border border-solid border-bdr-gray py-4 px-5  w-5/6 mx-auto relative ">
        ...
        <div className="absolute flex flex-col h-full justify-between top-0  -right-5 ">
          <IconButton
            ...
            onClick={()=>handleAddToBookmark(campaignHash,campaignTti)}
          >
